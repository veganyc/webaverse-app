...
class Avatar {
    ...
    this.localEulerHmd = new THREE.Euler(0, 0, 0, 'YXZ');
  ...
  update(timestamp, timeDiff) {
    ...
    const _updateHmdPosition = () => {
      ...
      // console.log("INPUTS HMD POS AND QUAT", currentPosition, currentQuaternion)
      
      const positionDiff = this.localVector.copy(this.lastPosition)
        .sub(currentPosition)
        .divideScalar(timeDiffS)
        .multiplyScalar(0.1);
      this.localEuler.setFromQuaternion(currentQuaternion, 'YXZ');
      this.localEuler.x = 0;
      this.localEuler.z = 0;
      this.localEuler.y += Math.PI;
      this.localEulerHmd.set(-this.localEuler.x, -this.localEuler.y, -this.localEuler.z, this.localEuler.order);
      positionDiff.applyEuler(this.localEulerHmd);
    ...
    const _overwritePose = poseName => {
      ...
      for (const spec of this.animationMappings) {
        ...
        if (!isPosition) {
          const src = poseAnimation.interpolants[atn];
    ...
    const _updateEyeTarget = () => {
      const eyePosition = getEyePosition(this.modelBones);
      const globalQuaternion = this.localQuaternion2.setFromRotationMatrix(
      ...
      this.modelBoneOutputs.Neck.matrixWorld.decompose(this.localVector, this.localQuaternion, this.localVector2);
      ...
      if (needsEyeTarget && !this.lastNeedsEyeTarget) {
        this.startEyeTargetQuaternion.copy(this.localQuaternion);
      ...
      if (needsEyeTarget) {
        this.localQuaternion.copy(this.startEyeTargetQuaternion)
          .slerp(globalQuaternion, cubicBezier(eyeTargetFactor));
        this.modelBoneOutputs.Neck.matrixWorld.compose(this.localVector, this.localQuaternion, this.localVector2)
        this.modelBoneOutputs.Neck.matrix.copy(this.modelBoneOutputs.Neck.matrixWorld)
          .premultiply(this.localMatrix2.copy(this.modelBoneOutputs.Neck.parent.matrixWorld).invert())
          .decompose(this.modelBoneOutputs.Neck.position, this.modelBoneOutputs.Neck.quaternion, this.localVector2);
      } else {
        if (eyeTargetFactor < 1) {
          this.localQuaternion2.copy(this.startEyeTargetQuaternion)
            .slerp(this.localQuaternion, cubicBezier(eyeTargetFactor));
          this.localMatrix.compose(this.localVector.set(0, 0, 0), this.localQuaternion2, this.localVector2.set(1, 1, 1))
            .premultiply(this.localMatrix2.copy(this.modelBoneOutputs.Neck.parent.matrixWorld).invert())
            .decompose(this.localVector, this.localQuaternion, this.localVector2);
        }
      }
    };
    ...
    const _updateEyeballTarget = () => {
      ...
      if (eyeballTarget && this.firstPersonCurves) {
        ...
        function lookAtEuler(euler) {
          ...
          if (leftEye) {
            if (srcX < 0.0) {
              this.localEulerHmd.x = -lookAtVerticalDownCurve.map(-srcX);
            } else {
              this.localEulerHmd.x = lookAtVerticalUpCurve.map(srcX);
            }

            if (srcY < 0.0) {
              this.localEulerHmd.y = -lookAtHorizontalInnerCurve.map(-srcY);
            } else {
              this.localEulerHmd.y = lookAtHorizontalOuterCurve.map(srcY);
            }
            this.localEulerHmd.x *= rotationFactor;
            this.localEulerHmd.y *= rotationFactor;
            this.localEulerHmd.y = Math.min(Math.max(this.localEulerHmd.y, -rotationRange), rotationRange);

            this.localEulerHmd.z = 0;
            this.localEulerHmd.order = 'YXZ';

            leftEye.quaternion.setFromEuler(this.localEulerHmd);
          ...
          if (rightEye) {
            ...
            rightEye.quaternion.setFromEuler(this.localEulerHmd);
            rightEye.updateMatrix();
          }
        }
        function lookAt(position) {
          calculateTargetEuler.call(this, this.localEuler, position);
          lookAtEuler.call(this, this.localEuler);
        }
    ...
    const _motionControl = () => {
      ...
      this.sdkInputs.leftGamepad.position.copy(this.inputs.leftGamepad.position).add(this.localVector.copy(this.handOffsetLeft).applyQuaternion(this.inputs.leftGamepad.quaternion));
      ...
      this.sdkInputs.rightGamepad.position.copy(this.inputs.rightGamepad.position).add(this.localVector.copy(this.handOffsetRight).applyQuaternion(this.inputs.rightGamepad.quaternion));
        ...
        this.springBoneManager && this.springBoneManager.springBoneGroupList.forEach(springBoneGroup => {
          springBoneGroup.forEach(springBone => {
            springBone._worldBoneLength = springBone.bone
              .localToWorld(this.localVector.copy(springBone._initialLocalChildPosition))
      ...
      if (this.options.fingers) {
        ...
        const _processFingerBones = left => {
          ...
          for (const fingerBone of fingerBones) {
            ...
            let setter;
            if (finger === 'thumb') {
              setter = (q, i) => q.setFromAxisAngle(this.localVector.set(0, left ? -1 : 1, 0), gamepadInput.grip * Math.PI * (i === 0 ? 0.125 : 0.25));
            } else if (finger === 'index') {
              setter = (q, i) => q.setFromAxisAngle(this.localVector.set(0, 0, left ? 1 : -1), gamepadInput.pointer * Math.PI * 0.5);
            } else {
              setter = (q, i) => q.setFromAxisAngle(this.localVector.set(0, 0, left ? 1 : -1), gamepadInput.grip * Math.PI * 0.5);
            }
            for (let i = 0; i < bones.length; i++) {
    ...
    // //sync animations with motion controls
    // if (this.getTopEnabled() || this.getHandEnabled(0) || this.getHandEnabled(1)) {
    //   _motionControl.call(this);
    // }

    // make sure avatar is facing correct direction
    this.localEuler.setFromQuaternion(this.inputs.hmd.quaternion, 'YXZ');
    this.localEuler.x = 0;
    this.localEuler.z = 0;
    this.localEuler.y += Math.PI;
    this.modelBoneOutputs.Root.quaternion.setFromEuler(this.localEuler);
    //make sure we're using the correct hight if we've scaled anything
    this.modelBoneOutputs.Root.position.copy(this.inputs.hmd.position)
      .sub(this.localVector.set(0, this.height, 0));
    ...
    if (game.debugMode && !this.debugMesh) {
      this.debugMesh = _makeDebugMesh(this);
